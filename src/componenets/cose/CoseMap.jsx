...
import { Map, Polyline, CustomOverlayMap, MapMarker} from "react-kakao-maps-sdk";
  ...
  return (
    <>
     ...
     <Header/>
        ...
        <BackDiv>
          <ReviseP onClick={() => navigate('/cose/revise/'+id)}>수정하기</ReviseP>
        ...
        <MapDiv>
            ...
            >
              {cose.map((coses,index) => {
                  return <CoseDetail cose={coses} index={index} key={index} length={cose.length}/>
              })}
              {cose?.map((path,index) => {
                return <CoseMark path={path} key={index} index={index} length={cose.length}/>
              })}
              <Polyline
                path={[cose?.map((path) =>(
                   {
